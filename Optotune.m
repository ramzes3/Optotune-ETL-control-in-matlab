    ...
    methods
        ...
        function lens = setModeUpperCurrent(lens,ci)
            if ci > lens.max_current || ci < 0
                ...
                ci = lens.max_current;
            else if ci < lens.modeLowerCurrent
                    lens.modeLowerCurrent = ci;
                end
            end
            set_i = (floor(ci*(lens.max_bin+1) / lens.max_current));
            LB = mod(set_i,256); %% low byte
        ...
        function lens = getModeUpperCurrent(lens)
            ...
            if strcmp(cellstr(char(lens.response(1:3))'), 'PUA')
                lens.modeUpperCurrent = (lens.response(4)*(hex2dec('ff')+1) + lens.response(5))*lens.max_current/lens.max_bin;
            else
                checkError(lens);
                if lens.error == true
        ...
        function lens = getModeLowerCurrent(lens)
            ...
            if strcmp(cellstr(char(lens.response(1:3))'), 'PLA')
                lens.modeLowerCurrent = (lens.response(4)*(hex2dec('ff')+1) + lens.response(5))*lens.max_current/lens.max_bin;  %% software current limit usually 292.84 mA;
            else
                checkError(lens);
                if lens.error == true
        ...
        function lens = getModeFrequency(lens)
            ...
            if strcmp(cellstr(char(lens.response(1:3))'), 'PFA')
                lens.modeFrequency = (lens.response(4) * 2^24 + lens.response(5) * 2^16 + lens.response(6) * 2^8 + lens.response(7))/1000;  %% software current limit usually 292.84 mA;
            else
                checkError(lens);
                if lens.error == true
